...
static void sdcardfs_netlink_process_packet(struct nlmsghdr *nl){
...
static void sdcardfs_netlink_recv_packet(struct sk_buff *__skb){
    struct sk_buff *skb;
    ...
    if(skb->len >= sizeof(struct nlmsghdr))	{
        ...
        if(nlhdr->nlmsg_len >= sizeof(struct nlmsghdr) &&	 __skb->len >= nlhdr->nlmsg_len)	{
            sdcardfs_netlink_process_packet(nlhdr);
        }
    }	else
        printk(KERN_ALERT "Kernel receive msg length error!\n");
}
static struct netlink_kernel_cfg fo_netlink_cfg = {
        .input = sdcardfs_netlink_recv_packet
};

int sdcardfs_init_file_observer(void) {
    if(!gEnable)
        return 0;
...
int sdcardfs_exit_file_observer(void) {
    if(!gEnable)
        return 0;
...
static int sdcardfs_nl_send_msg(const u8 *data, int len)    {
    struct nlmsghdr *rep;
    u8 *res;
    struct sk_buff *skb;
...
static const char* sdcardfs_getMountPoint(struct dentry *dentry) {
...
static size_t sdcardfs_getDentryFullPath(struct dentry *dentry, char* buf, size_t buffSize) {
    const char* mount_point = "";
    size_t mount_point_len = 0;

    memset(buf, 0, buffSize);
    mount_point = sdcardfs_getMountPoint(dentry);
...
static size_t  sdcardfs_createMsg(FO_OP op, struct inode *inode, struct dentry *dentry,
                                                         struct sdcardfs_file_creator* creator, char* buf, size_t buffSize,
    ...
    if(inode != NULL) {
        ...
        if(dentry != NULL && fm_path != NULL) {
            len = sdcardfs_getDentryFullPath(dentry, (void*)offset, buffSize-(offset-buf));
        ...
        if(old_dentry != NULL && fm_oldpath != NULL) {
            len = sdcardfs_getDentryFullPath(old_dentry, (void*)offset, buffSize-(offset-buf));
        ...
        if(new_dentry != NULL && fm_newpath != NULL) {
            len = sdcardfs_getDentryFullPath(new_dentry, (void*)offset, buffSize-(offset-buf));
...
int sdcardfs_post_file_create(struct dentry *dentry)  {
    char* data;
    size_t len = 0;
    ...
    #if DEBUG
    printk("SDCARDFS thread name:%s\n", current->comm);
    #endif

    data = (char*)kmalloc(NL_MSG_MAX, GFP_KERNEL);
    if(data != NULL) {
        len = sdcardfs_createMsg(OP_CREATE, dentry->d_inode, dentry, NULL, data, NL_MSG_MAX, NULL, NULL);
        sdcardfs_nl_send_msg(data, len);
        kfree(data);
    }
...
#if 0
int sdcardfs_post_file_open(struct file *file)  {

    struct sdcardfs_inode_info* inode_info;
    struct sdcardfs_file *file_info = file->private_data;
    ...
    inode_info = (struct sdcardfs_inode_info*)file->f_path.dentry->d_inode->i_private;//SDCARDFS_I(file->f_path.dentry->d_inode);
    printk("-222 open--->%p\n", inode_info);
    if(inode_info == NULL) {
        return -1;
    }
    if(inode_info->mask|SDCARDFS_FS_CREATE) {
        printk("post_file_open  %lu  \n", file->f_path.dentry->d_inode->i_ino);
        file_info->mask|= SDCARDFS_FS_CREATE;
        inode_info->mask &= ~SDCARDFS_FS_CREATE;
        if(inode_info->mask|SDCARDFS_RELEASE_PRIVATE) {
...
int sdcardfs_post_file_write(struct file *file)  {
    struct sdcardfs_file_info* info;
    ...
    info = (struct sdcardfs_file_info*)file->private_data;
    info->mask |= SDCARDFS_FS_MODIFY;

    return 0;
}
int sdcardfs_post_file_mkdir(struct inode *dir, struct dentry *dentry) {
    char* data;
    size_t len = 0;
    ...
    printk("====mkdir %s\n", dentry->d_name.name);
     ...
     if(dentry != NULL && dir != NULL) {
        data = (char*)kmalloc(NL_MSG_MAX, GFP_KERNEL);
        if(data != NULL) {
            len = sdcardfs_createMsg(OP_MKDIR, dir, dentry, NULL, data, NL_MSG_MAX, NULL, NULL);
            sdcardfs_nl_send_msg(data, len);
            kfree(data);
        }
    }
...
int sdcardfs_post_file_rmdir(struct inode *dir, struct dentry *dentry) {
    char* data;
    size_t len = 0;
    ...
    if(dentry != NULL && dentry->d_inode != NULL) {
        data = (char*)kmalloc(NL_MSG_MAX, GFP_KERNEL);
        if(data != NULL) {
            len = sdcardfs_createMsg(OP_RMDIR, dir, dentry, NULL, data, NL_MSG_MAX, NULL, NULL);
            sdcardfs_nl_send_msg(data, len);
            kfree(data);
        }
    }
...
int sdcardfs_post_file_rename(struct inode *old_dir, struct dentry *old_dentry,
    ...
    if(!gEnable)
     ...
     if(old_dir != NULL && old_dentry != NULL && new_dir != NULL && new_dentry != NULL) {
        ...
        if(data != NULL) {
            len = sdcardfs_createMsg(OP_RENAME, old_dentry->d_inode, NULL, NULL, data,NL_MSG_MAX, old_dentry, new_dentry);
            sdcardfs_nl_send_msg(data, len);
            kfree(data);
        }
    }
...
int sdcardfs_post_file_unlink(struct inode *dir, struct dentry *dentry) {
    char* data;
    size_t len = 0;
    ...
    if(dentry != NULL && dentry->d_inode != NULL) {
        data = (char*)kmalloc(NL_MSG_MAX, GFP_KERNEL);
        if(data != NULL) {
            len = sdcardfs_createMsg(OP_UNLINK, dentry->d_inode, dentry, NULL, data, NL_MSG_MAX, NULL, NULL);
            sdcardfs_nl_send_msg(data, len);
            kfree(data);
        }
    }
...
int sdcardfs_post_file_release(struct inode *inode, struct file *file) {
    char* data;
    struct sdcardfs_file_info* ff;
    size_t len = 0;
    ...
    if(file != NULL) {
        ff = (struct sdcardfs_file_info*)file->private_data;
        if(file->f_path.dentry != NULL &&  ((ff->mask & SDCARDFS_FS_MODIFY) )) {
            data = (char*)kmalloc(NL_MSG_MAX, GFP_KERNEL);
            if(data != NULL) {
                #if 1
                if(inode->i_ino == g_last_msg.ino && ((jiffies - g_last_msg.time) < HZ)) {
                    memset(&g_last_msg.data, 0, NL_MSG_MAX);
                    g_last_msg.data_len = sdcardfs_createMsg(OP_CLOSE_WRITE, inode, file->f_path.dentry, &ff->creator, g_last_msg.data, NL_MSG_MAX, NULL, NULL);
                    ...
                    if(g_last_msg.is_valid) {
                        sdcardfs_nl_send_msg(g_last_msg.data, g_last_msg.data_len);
                        g_last_msg.is_valid = false;
                    }

                    g_last_msg.ino = inode->i_ino;
                    g_last_msg.time = jiffies;
                    len = sdcardfs_createMsg(OP_CLOSE_WRITE, inode, file->f_path.dentry, &ff->creator, data, NL_MSG_MAX, NULL, NULL);
                    sdcardfs_nl_send_msg(data, len);
                }
                #else
                len = sdcardfs_createMsg(OP_CLOSE_WRITE, inode, file->f_path.dentry, &ff->creator, data, NL_MSG_MAX, NULL, NULL);
...
bool sdcardfs_do_fileobserver_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
       void __user *ubuf = (void __user *)arg;
       struct sdcardfs_file_info *ff = file->private_data;

        #if DEBUG
        printk("SDCARDFS ioctl be called  cmd:%u\n", cmd);
        #endif

        if(cmd == SDCARDFS_FILEOBSERVER_IOCTL_CMD && ff != NULL) {
            if (copy_from_user(&ff->creator, ubuf, sizeof(struct sdcardfs_file_creator))) {
