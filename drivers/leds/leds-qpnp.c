...
static int qpnp_rgb_set(struct qpnp_led_data *led)
{
	...
	if (led->cdev.brightness) {
		...
		if (led->rgb_cfg->pwm_cfg->mode == PWM_MODE) {
			//ADD by wanghaiqing ,deal for touch screen continuous,left led and right led doesnt't light sometimes
			#ifdef CONFIG_ZTEMT_BREATH_LEDS
			if(led->rgb_cfg->is_auto_breath){
                        //resovle for rom [setting]->[display]->[breathlight]->[breath light brightness] doesn't work problem
...
static int  qpnp_led_action_trigger(struct qpnp_led_data *led)
{
    ...
    //NUBIA ADD FOR RESOLVE NOTIFICATIONS  LED BLINK PROBLEM
    if (pwm_cfg->pwm_enabled) {
        pwm_disable(pwm_cfg->pwm_dev);
        pwm_cfg->pwm_enabled = 0;
    }
