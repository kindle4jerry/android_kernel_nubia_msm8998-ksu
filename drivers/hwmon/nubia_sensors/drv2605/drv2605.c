...
static ssize_t drv2605_vib_max_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", MAX_VIBE_STRENGTH);
}

static ssize_t drv2605_vib_default_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", DEF_VIBE_STRENGTH);
}

static ssize_t drv2605_vib_level_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", vibe_strength);
}

static ssize_t drv2605_vib_level_store(struct device *dev,
...
static DEVICE_ATTR(vtg_min, S_IRUGO, drv2605_vib_min_show, NULL);
static DEVICE_ATTR(vtg_max, S_IRUGO, drv2605_vib_max_show, NULL);
static DEVICE_ATTR(vtg_default, S_IRUGO, drv2605_vib_default_show, NULL);
static DEVICE_ATTR(vtg_level, S_IRUGO | S_IWUSR, drv2605_vib_level_show, drv2605_vib_level_store);
...
static ssize_t dev2605_write(struct file* filp, const char* buff, size_t len, loff_t* off)
{
	...
	char *temp = NULL;
	if(len > 0){
       temp =(char *) kzalloc(sizeof(char)*len, GFP_KERNEL);
	   if(temp == NULL){
           dev_err(pDrv2605data->device,
							   "failed to allocate %ld bytes through kzalloc \n",len);
		   return -EFAULT;
	   }
	...
	switch(temp[0])
	{
		...
		case HAPTIC_CMDID_PLAY_EFFECT_SEQUENCE:
		{
			...
			memcpy(&pDrv2605data->sequence, &temp[1], len - 1);
		...
		case HAPTIC_CMDID_PATTERN_RTP:
		{
			...
			pDrv2605data->vibration_time = (int)((((int)temp[2])<<8) | (int)temp[1]);
			pDrv2605data->silience_time = (int)((((int)temp[4])<<8) | (int)temp[3]);
			strength = temp[5];
			pDrv2605data->repeat_times = temp[6];
		...
		case HAPTIC_CMDID_RTP_SEQUENCE:
		{
			...
			if(((len-1)%2) == 0){
				...
				if((pDrv2605data->RTPSeq.RTPCounts <= MAX_RTP_SEQ)&&(pDrv2605data->RTPSeq.RTPCounts>0)){
					memcpy(pDrv2605data->RTPSeq.RTPData, &temp[1], pDrv2605data->RTPSeq.RTPCounts*2);
		...
		case HAPTIC_CMDID_REG_READ:
		{
			if(len == 2){
				...
				pDrv2605data->ReadBuff[0] = drv2605_reg_read(pDrv2605data, temp[1]);
			}else if(len == 3){
				pDrv2605data->ReadLen = (temp[2]>MAX_READ_BYTES)?MAX_READ_BYTES:temp[2];
				drv2605_bulk_read(pDrv2605data, temp[1], pDrv2605data->ReadLen, pDrv2605data->ReadBuff);
			}else{
		...
		case HAPTIC_CMDID_REG_SETBIT:
		{
			int i=1;
			for(i=1; i< len; ){
				drv2605_set_bits(pDrv2605data, temp[i], temp[i+1], temp[i+2]);
				i += 3;
			}
			break;
		}
