...
#define MISC_ICL_STATUS_REG				0x1607
#define MISC_AICL_STATUS_REG			0x160A
...
static void spmi_dump_regs(struct neo_battery_data *chip)
{
	...
	for(i = 0; i < table_size; i++){
		...
		printk("%s[%x 0x%02x] ", regs_table[i].name, regs_table[i].reg, reg);
	}
	printk("\n");
}
...
pr_err("NEO:neo_battery_probe start\n");
	...
	schedule_delayed_work(&chip->soc_monitor_work, msecs_to_jiffies(6 * MONITOR_SOC_WAIT_MS));
